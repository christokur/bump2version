...
def _mock_calls_to_string(called_mock):
    return [
        "{}|{}|{}".format(name, args[0] if len(args) > 0 else args, repr(kwargs) if len(kwargs) > 0 else "")
...
""".format(
        version=version, file=file, message=message
    )
...
def test_dirty_work_dir(tmpdir, vcs):
    ...
    log_capture.check_present(
        (
            ...
            "{}\n"
            "\n"
            "Use --allow-dirty to override this if you know what you're doing.".format(vcs_name, vcs_output),
        )
    )
...
@pytest.mark.xfail(not config_parser_handles_utf8, reason="old ConfigParser uses non-utf-8-strings internally")
...
def test_non_matching_search_does_not_modify_file(tmpdir):
    ...
    * bullet point B
    ...
    with pytest.raises(
        exceptions.VersionNotFoundException, match="Did not find 'Not-yet-released' in file: 'CHANGELOG.md'"
    ):
        main(["patch", "--verbose"])
