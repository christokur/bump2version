...
RE_DETECT_SECTION_TYPE = re.compile(
    r"^bumpversion:" r"((?P<file>file|glob)(\s*\(\s*(?P<file_suffix>[^\):]+)\)?)?|(?P<part>part)):" r"(?P<value>.+)",
)
...
def main(original_args=None):
    ...
    if vcs:
        context = _commit_to_vcs(
            files, context, config_file, config_file_exists, vcs, args, current_version, new_version
        )
